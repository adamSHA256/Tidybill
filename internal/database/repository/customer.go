...
func (r *CustomerRepository) Create(c *model.Customer) error {
	...
	_, err := r.db.Exec(`
		...
		c.EmailCustomTemplate, c.EmailSubjectTemplate, c.EmailBodyTemplate, c.CreatedAt, c.UpdatedAt)
	return err
}
...
func (r *CustomerRepository) Update(c *model.Customer) error {
	...
	_, err := r.db.Exec(`
		...
		c.Email, c.Phone, c.DefaultVATRate, c.DefaultDueDays, c.Notes,
		c.EmailCustomTemplate, c.EmailSubjectTemplate, c.EmailBodyTemplate, c.UpdatedAt, c.ID)
	return err
}
...
func (r *CustomerRepository) List() ([]*model.Customer, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&c.ID, &c.Name, &c.Street, &c.City, &c.ZIP, &c.Region, &c.Country,
			...
			&c.Notes, &c.EmailCustomTemplate, &c.EmailSubjectTemplate, &c.EmailBodyTemplate,
