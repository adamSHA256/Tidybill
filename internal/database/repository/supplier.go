...
func (r *SupplierRepository) Create(s *model.Supplier) error {
	...
	_, err := r.db.Exec(`
		INSERT INTO suppliers (id, name, street, city, zip, country, ico, dic, ic_dph, phone, email, website,
			logo_path, is_vat_payer, is_default, invoice_prefix, notes, language, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		s.ID, s.Name, s.Street, s.City, s.ZIP, s.Country, s.ICO, s.DIC, s.ICDPH, s.Phone, s.Email, s.Website,
...
func (r *SupplierRepository) Update(s *model.Supplier) error {
	s.UpdatedAt = time.Now()
	_, err := r.db.Exec(`
		UPDATE suppliers SET name=?, street=?, city=?, zip=?, country=?, ico=?, dic=?, ic_dph=?, phone=?, email=?, website=?,
			logo_path=?, is_vat_payer=?, is_default=?, invoice_prefix=?, notes=?, language=?, updated_at=?
		WHERE id=?`,
		s.Name, s.Street, s.City, s.ZIP, s.Country, s.ICO, s.DIC, s.ICDPH, s.Phone, s.Email, s.Website,
...
func (r *SupplierRepository) GetByID(id string) (*model.Supplier, error) {
	s := &model.Supplier{}
	err := r.db.QueryRow(`
		SELECT id, name, street, city, zip, country, ico, dic, ic_dph, phone, email, website,
			logo_path, is_vat_payer, is_default, invoice_prefix, notes, language, created_at, updated_at
...
func (r *SupplierRepository) List() ([]*model.Supplier, error) {
	...
	for rows.Next() {
		s := &model.Supplier{}
		if err := rows.Scan(&s.ID, &s.Name, &s.Street, &s.City, &s.ZIP, &s.Country, &s.ICO, &s.DIC, &s.ICDPH,
