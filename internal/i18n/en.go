package i18n

var messagesEN = map[string]string{
	// ── App ──────────────────────────────────────────────────────────────
	"app.error_config":   "Configuration error: %v",
	"app.error_database": "Database error: %v",
	"app.error_general":  "Error: %v",
	"app.goodbye":        "Goodbye!",

	// ── Header ──────────────────────────────────────────────────────────
	"header.no_company": "Not set",
	"header.company":    "Company:",

	// ── Main menu ───────────────────────────────────────────────────────
	"menu.create_invoice":      "Create new invoice",
	"menu.create_from_existing": "Create invoice from existing",
	"menu.list_invoices":       "Invoice list",
	"menu.unpaid_invoices":     "Unpaid invoices",
	"menu.unpaid_count":        "%d unpaid",
	"menu.overdue_count":       "%d overdue",
	"menu.customers":           "Customers",
	"menu.items_catalog":       "Item catalog",
	"menu.suppliers":           "Suppliers (your companies)",
	"menu.sync":                "Sync / Import / Export",
	"menu.pdf_templates":       "PDF templates",
	"menu.settings":            "Settings",
	"menu.quit":                "Quit",

	// ── Headings ────────────────────────────────────────────────────────
	"heading.suppliers":         "SUPPLIERS (your companies)",
	"heading.new_supplier":      "NEW SUPPLIER",
	"heading.supplier_detail":   "SUPPLIER: %s",
	"heading.edit_supplier":     "EDIT SUPPLIER",
	"heading.new_bank_account":  "NEW BANK ACCOUNT",
	"heading.customers":         "CUSTOMERS",
	"heading.new_customer":      "NEW CUSTOMER",
	"heading.customer_detail":   "CUSTOMER: %s",
	"heading.edit_customer":     "EDIT CUSTOMER",
	"heading.customer_invoices": "INVOICES: %s",
	"heading.new_invoice":       "NEW INVOICE",
	"heading.invoice_summary":   "INVOICE SUMMARY",
	"heading.invoice_list":      "INVOICE LIST",
	"heading.unpaid_invoices":   "UNPAID INVOICES",
	"heading.invoice_detail":    "INVOICE %s",
	"heading.items_catalog":     "ITEM CATALOG",
	"heading.sync":              "SYNC / IMPORT / EXPORT",
	"heading.pdf_templates":     "PDF TEMPLATES",
	"heading.settings":          "SETTINGS",

	// ── Prompts ─────────────────────────────────────────────────────────
	"prompt.choose_option":       "Choose an option",
	"prompt.choice":              "Choice",
	"prompt.back_hint":           "(0=back)",
	"prompt.press_enter":         "Press Enter to continue...",
	"prompt.leave_empty_hint":    "(Leave empty to keep current value)",
	"prompt.enter_0_back":        "(Enter 0 to go back)",
	"prompt.enter_0_back_anytime": "(Enter 0 anytime to go back)",
	"prompt.company_name":        "Company name",
	"prompt.name":                "Name",
	"prompt.street":              "Street and number",
	"prompt.street_short":        "Street",
	"prompt.city":                "City",
	"prompt.zip":                 "ZIP code",
	"prompt.country":             "Country",
	"prompt.ico":                 "Company ID",
	"prompt.dic":                 "Tax ID",
	"prompt.dic_with_hint":       "Tax ID (if VAT payer, otherwise leave empty)",
	"prompt.ic_dph":              "VAT ID (IČ DPH)",
	"prompt.phone":               "Phone",
	"prompt.email":               "E-mail",
	"prompt.website":             "Website",
	"prompt.invoice_number":      "Invoice number",
	"prompt.invoice_prefix":      "Invoice number prefix",
	"prompt.invoice_prefix_short": "Invoice prefix",
	"prompt.account_name":        "Account name",
	"prompt.account_number":      "Account number (e.g. 1234567890/0100)",
	"prompt.iban":                "IBAN",
	"prompt.swift":               "SWIFT/BIC",
	"prompt.currency":            "Currency",
	"prompt.qr_type":             "QR code type",
	"prompt.qr_type_spayd":       "SPAYD — Czech format",
	"prompt.qr_type_epc":         "EPC — outside CZ/SK, EUR only",
	"prompt.qr_type_pbs":         "Pay by Square — Slovak format",
	"prompt.qr_type_none":        "None",
	"prompt.region":              "Region (optional)",
	"prompt.due_days":            "Due days",
	"prompt.default_due_days":    "Due days",
	"prompt.notes":               "Notes",
	"prompt.item_description":    "Item description",
	"prompt.quantity":            "Quantity",
	"prompt.unit":                "Unit",
	"prompt.unit_price":          "Unit price",
	"prompt.vat_rate":            "VAT %",
	"prompt.select_supplier":     "Select supplier (your company):",
	"prompt.select_customer":     "Select customer:",
	"prompt.select_bank_account": "Select bank account:",
	"prompt.select_invoice":      "Select invoice for details",
	"prompt.select_invoice_short": "Select invoice",
	"prompt.select_status":       "Select new status:",
	"prompt.select_for_detail":   "Select number for details (0 = back)",
	"prompt.search_name_ico":     "Search (name or company ID)",
	"prompt.add_item":            "Add item:",
	"prompt.invoice_notes":       "Invoice notes (printed on PDF)",
	"prompt.due_date_confirm":    "Due date (DD.MM.YYYY)",
	"prompt.issue_date_confirm":  "Issue date (DD.MM.YYYY)",

	// ── Labels ──────────────────────────────────────────────────────────
	"label.address":              "Address: %s, %s %s, %s",
	"label.address_short":        "Address:  %s, %s %s",
	"label.company":              "Company:  %s",
	"label.ico":                  "ID:       %s",
	"label.dic":                  "Tax ID:   %s",
	"label.ic_dph":               "IČ DPH:   %s",
	"label.phone":                "Phone:   %s",
	"label.phone_full":           "Phone:     %s",
	"label.email":                "E-mail:  %s",
	"label.email_full":           "E-mail:    %s",
	"label.website":              "Website: %s",
	"label.prefix":               "Prefix:  %s",
	"label.is_default":           "Default: %v",
	"label.bank_accounts":        "Bank accounts:",
	"label.default_upper":        "[DEFAULT]",
	"label.default_lower":        "[default]",
	"label.account":              "Account:  %s",
	"label.account_with_currency": "Account: %s (%s)",
	"label.region":               "Region:    %s",
	"label.due_days":             "Due:       %d days",
	"label.notes":                "Notes:     %s",
	"label.internal_notes":       "Internal notes: %s",
	"label.invoice_number":       "Invoice: %s",
	"label.invoice_number_full":  "Invoice number: %s",
	"label.customer":             "Customer:      %s",
	"label.customer_short":       "Customer: %s",
	"label.supplier":             "Supplier: %s",
	"label.date":                 "Date:          %s",
	"label.due_date":             "Due date:      %s",
	"label.due_date_short":       "Due date: %s",
	"label.issued":               "Issued:      %s",
	"label.variable_symbol":      "Ref:         %s",
	"label.status":               "Status:      %s %s",
	"label.items":                "Items:",
	"label.subtotal":             "Subtotal:",
	"label.vat":                  "VAT:",
	"label.total":                "TOTAL:",
	"label.current_total":        "Current total: %.2f %s",
	"label.overdue_days":         "OVERDUE %d days",
	"label.due":                  "due",

	// ── Actions ─────────────────────────────────────────────────────────
	"action.new_supplier":     "N) New supplier",
	"action.new_customer":     "N) New customer",
	"action.new_item":         "N) New item",
	"action.back":             "0) Back",
	"action.search":           "H) Search",
	"action.edit_details":     "E) Edit details",
	"action.add_bank_account": "B) Add bank account",
	"action.set_default":      "D) Set as default",
	"action.show_invoices":    "F) Show invoices",
	"action.notes":            "N) Notes",
	"action.internal_notes":   "N) Internal notes",
	"action.delete_customer":  "X) Delete customer",
	"action.done":             "D) Done",
	"action.cancel_invoice":   "0) Cancel invoice",
	"action.save_invoice":     "U) Save invoice",
	"action.cancel":           "Z) Cancel",
	"action.generate_pdf":     "G) Generate PDF",
	"action.open_pdf":         "O) Open PDF",
	"action.change_status":    "S) Change status",
	"action.mark_paid":        "P) Mark as paid",
	"action.delete_invoice":   "X) Delete invoice",

	// ── Confirm ─────────────────────────────────────────────────────────
	"confirm.yes_no":            "[Y/n]",
	"confirm.yes_default":       "y",
	"confirm.yes_options":       "a,y",
	"confirm.vat_payer":         "Are you a VAT payer?",
	"confirm.save_data":         "Save this data?",
	"confirm.add_bank_account":  "Add a bank account?",
	"confirm.set_as_default":    "Set as default?",
	"confirm.delete_customer":   "Really delete customer?",
	"confirm.cancel_invoice":    "Really cancel invoice creation?",
	"confirm.generate_pdf":      "Generate PDF?",
	"confirm.open_pdf":          "Open PDF?",
	"confirm.mark_as_sent":      "Mark as sent?",
	"confirm.delete_invoice":    "Really delete invoice?",

	// ── Success ─────────────────────────────────────────────────────────
	"success.profile_created":   "Profile created successfully!",
	"success.supplier_created":  "Supplier created",
	"success.supplier_updated":  "Supplier updated",
	"success.set_as_default":    "Set as default",
	"success.account_added":     "Account added",
	"success.customer_created":  "Customer created",
	"success.customer_updated":  "Customer updated",
	"success.notes_saved":       "Notes saved",
	"success.customer_deleted":  "Customer deleted",
	"success.invoice_created":   "Invoice %s created!",
	"success.invoice_paid":      "Invoice marked as paid",
	"success.invoice_deleted":   "Invoice deleted",
	"success.status_changed":    "Status changed",
	"success.pdf_created":       "PDF created: %s",

	// ── Error ───────────────────────────────────────────────────────────
	"error.prefix":              "Error:",
	"error.name_required":       "Name is required",
	"error.name_required_lower": "name is required",
	"error.cancelled_by_user":   "cancelled by user",
	"error.save_failed":         "failed to save: %w",
	"error.save_account_failed": "failed to save account: %w",
	"error.no_supplier":         "No supplier configured",
	"error.no_bank_account":     "No bank account configured",
	"error.invoice_no_items":    "Invoice must have at least one item",
	"error.load_supplier":       "Failed to load supplier",
	"error.load_customer":       "Failed to load customer",
	"error.load_bank_account":   "Failed to load bank account",
	"error.load_items":          "Failed to load items",
	"error.open_file":           "Failed to open file: %v",
	// ── Info ────────────────────────────────────────────────────────────
	"info.no_customers":        "You have no customers yet.",
	"info.no_customers_create": "You have no customers. Create a new one:",
	"info.nothing_found":       "Nothing found.",
	"info.no_invoices":         "No invoices.",
	"info.all_paid":            "All invoices are paid!",
	"info.invoice_not_saved":   "Invoice was not saved.",
	"info.generating_pdf":      "Generating PDF...",
	"info.coming_soon":         "Coming soon...",
	"info.feature_coming":      "This feature will be available in the next version.",

	// ── Status ──────────────────────────────────────────────────────────
	"status.draft":     "Draft",
	"status.created":   "Created",
	"status.sent":      "Sent",
	"status.paid":      "Paid",
	"status.overdue":   "Overdue",
	"status.cancelled": "Cancelled",

	// ── Wizard ──────────────────────────────────────────────────────────
	"wizard.welcome_title":    "WELCOME TO TIDYBILL",
	"wizard.no_data":          "No data found.",
	"wizard.setup_prompt":     "Let's set up your first company profile.",
	"wizard.supplier_details": "Supplier details (your company)",
	"wizard.bank_account":     "Bank account",
	"wizard.pdf_directory":    "PDF output",
	"wizard.pdf_dir_prompt":   "Where should we save generated PDF invoices?",
	"wizard.summary":          "Summary",
	"label.pdf_dir":           "PDFs:     %s",

	// ── Settings ────────────────────────────────────────────────────────
	"settings.change_language":  "Change application language",
	"settings.current_language": "Current language:",
	"settings.current_currency": "Default currency:",
	"settings.current_due_days": "Default due days:",
	"settings.change_currency":  "Change default currency",
	"settings.change_due_days":  "Change default due days",
	"settings.days":             "days",
	"success.language_changed":  "Language changed to: %s",
	"success.currency_changed":  "Default currency changed to: %s",
	"success.due_days_changed":  "Default due days changed to: %s days",

	// ── Defaults ────────────────────────────────────────────────────────
	"default.main_account": "Main account",
	"default.unit_pcs":     "pcs",
	"unit.ks":              "pcs",
	"unit.hod":             "hr",
	"unit.den":             "day",

	// ── PDF ─────────────────────────────────────────────────────────────
	"pdf.invoice_title":        "Invoice - Tax Document %s",
	"pdf.invoice_title_simple": "Invoice %s",
	"pdf.supplier":         "SUPPLIER",
	"pdf.customer":         "CUSTOMER",
	"pdf.ico":              "Company ID: %s",
	"pdf.dic":              "Tax ID: %s",
	"pdf.ic_dph":           "IČ DPH: %s",
	"pdf.phone":            "Phone: %s",
	"pdf.email":            "E-mail: %s",
	"pdf.country":          "Country: %s",
	"pdf.issue_date":       "Issue date:",
	"pdf.due_date":         "Due date:",
	"pdf.payment_method":   "Payment method:",
	"pdf.bank_transfer":    "Bank transfer",
	"pdf.bank_account":     "Bank account:",
	"pdf.iban":             "IBAN:",
	"pdf.variable_symbol":  "Reference:",
	"pdf.amount_due":       "Amount due",
	"pdf.col_description":  "Description",
	"pdf.col_quantity":     "Qty",
	"pdf.col_unit_price":   "Unit price",
	"pdf.col_vat_rate":     "VAT %",
	"pdf.col_vat":          "VAT",
	"pdf.col_total":        "Total",
	"pdf.subtotal":         "Subtotal:",
	"pdf.vat_total":        "VAT:",
	"pdf.total":            "TOTAL:",
	"pdf.qr_payment":       "QR code for payment",
	"pdf.vat_payer":        "VAT payer",
	"pdf.not_vat_payer":    "Not a VAT payer",
	"pdf.invoice_msg":              "Invoice %s",
	"pdf.taxable_date":             "Tax date",
	"pdf.variable_symbol_short":    "Ref",

	// ── Items Catalog ───────────────────────────────────────────────────
	"heading.new_item":            "NEW CATALOG ITEM",
	"heading.item_detail":         "ITEM: %s",
	"heading.edit_item":           "EDIT ITEM",
	"heading.select_from_catalog": "SELECT FROM CATALOG",

	"prompt.default_price":        "Default price",
	"prompt.default_unit":         "Default unit",
	"prompt.default_vat":          "Default VAT %",
	"prompt.category":             "Category",
	"prompt.search_items":         "Search items (name or category)",
	"prompt.select_category":      "Existing categories:",
	"prompt.create_new_item_yn":   "Create a new item?",

	"action.new_item_catalog":     "N) New item",
	"action.from_catalog":         "K) From catalog",
	"action.search_items":         "H) Search",
	"action.delete_item":          "X) Delete item",
	"action.new_item_inline":      "N) Create new item",
	"action.new_category":         "  or enter a new category",
	"action.next_page":            "> Next page",
	"action.prev_page":            "< Previous page",

	"label.description":           "Description",
	"label.default_price":         "Default price",
	"label.default_unit":          "Unit",
	"label.default_vat":           "VAT",
	"label.category":              "Category",
	"label.usage_count":           "Usage",
	"label.last_price":            "Last price",
	"label.last_customer":         "Last customer",
	"label.deleted_customer":      "(deleted customer)",
	"label.customer_items":        "Your items for this customer:",
	"label.global_items":          "Most used items:",
	"label.from_catalog":          "From catalog",
	"label.catalog_price":         "catalog",
	"label.global":                "total",
	"label.page":                  "Page",

	"info.no_items":               "Item catalog is empty.",
	"info.no_catalog_items":       "No items in catalog.",
	"info.create_item_first":      "Create your first item.",

	"success.item_created":        "Item created",
	"success.item_updated":        "Item updated",
	"success.item_deleted":        "Item deleted",

	"confirm.delete_item":         "Really delete item?",
	"confirm.create_duplicate":    "Item with this description already exists. Create duplicate?",

	"warning.item_exists":         "Existing item: \"%s\" (price: %.2f)",

	"error.description_required":  "Description is required",
	"error.input_too_long":        "Input too long (max %d characters)",

	// ── Recent Items ────────────────────────────────────────────────────
	"label.recent_items":          "Recently used:",

	// ── Invoice Filters ─────────────────────────────────────────────────
	"action.filter":               "F) Filter",
	"prompt.filter_by":            "Filter by:",
	"action.filter_status":        "S) By status",
	"action.filter_customer":      "C) By customer",
	"action.filter_date":          "D) By date range",
	"action.reset_filter":         "R) Reset filters",
	"prompt.date_from":            "From date (DD.MM.YYYY)",
	"prompt.date_to":              "To date (DD.MM.YYYY)",
	"label.active_filter":         "[Filter: %s]",
	"info.no_invoices_filter":     "No invoices match the filter.",

	// ── Duplicate Invoice ───────────────────────────────────────────────
	"heading.duplicate_invoice":   "DUPLICATE INVOICE",
	"label.source_invoice":        "Source: %s | %s | %.2f %s",
	"label.source_items":          "  %d items: %s",
	"action.quick_duplicate":      "Q) Quick duplicate (new number + dates, everything else same)",
	"action.edit_before_save":     "E) Edit before saving",
	"action.edit_item":            "E) Edit item",
	"action.remove_all_items":     "R) Remove all items",
	"action.continue":             "C) Continue",
	"prompt.which_item":           "Which item?",
	"confirm.remove_all_items":    "Remove all items?",

	// ── Edit Draft Invoice ──────────────────────────────────────────────
	"action.edit_invoice":         "E) Edit invoice",
	"heading.edit_invoice":        "EDIT INVOICE %s",
	"action.change_customer":      "C) Change customer",
	"action.change_due_date":      "D) Change due date",
	"action.change_notes":         "N) Change notes",
	"action.edit_items":           "I) Edit items",
	"action.add_item_edit":        "A) Add item",
	"action.remove_item":          "X) Remove item",
	"label.remove":                "Remove",
	"prompt.item_number":          "Item number to remove",
	"prompt.new_due_date":         "New due date (DD.MM.YYYY)",
	"confirm.remove_item":         "Remove this item?",
	"success.invoice_updated":     "Invoice updated",
	"info.draft_only":             "Only draft invoices can be edited.",

	// ── Bank Account Management ─────────────────────────────────────────
	"action.manage_accounts":      "A) Manage bank accounts",
	"info.no_bank_accounts":       "No bank accounts.",
	"heading.bank_accounts_mgmt":  "BANK ACCOUNTS",
	"heading.edit_bank_account":   "EDIT BANK ACCOUNT: %s",
	"action.edit_account_details": "E) Edit details",
	"action.set_account_default":  "D) Set as default",
	"action.delete_account":       "X) Delete account",
	"confirm.delete_account":      "Really delete this account?",
	"success.account_updated":     "Account updated",
	"success.account_deleted":     "Account deleted",
	"error.last_account":          "Cannot delete the last account",
	"error.account_in_use":        "Account is used by %d invoices, cannot delete",
	"prompt.select_account":       "Select account to edit",
	"prompt.back":                 "Back",

	// ── PDF Templates (new keys) ────────────────────────────────────────
	"pdf.from":            "FROM",
	"pdf.for":             "FOR",
	"pdf.issued_for":      "Issued for",
	"pdf.payment_info":    "Payment info",
	"pdf.dates":           "Important dates",
	"pdf.col_unit":        "Unit",

	// ── Templates Menu ──────────────────────────────────────────────────
	"templates.available":       "Available templates",
	"templates.active":          "ACTIVE",
	"templates.actions":         "Actions",
	"templates.preview_all":     "Generate all previews (Lorem Ipsum)",
	"templates.preview_one":     "Generate preview for selected template",
	"templates.set_default":     "Set default template",
	"templates.edit":            "Edit template (name, logo, QR, notes)",
	"templates.qr_settings":     "QR code type settings for bank account",
	"templates.select_number":   "Enter template number",
	"templates.generating_all":  "Generating all template previews...",
	"templates.all_generated":   "All previews generated",
	"templates.open_previews":   "Open generated previews?",
	"templates.generating_one":  "Generating preview for %s...",
	"templates.preview_created": "Preview created",
	"templates.choose_default":  "Select new default template:",
	"templates.default_set":     "Default template set",
	"templates.editing":         "Editing template",
	"templates.name":            "Template name",
	"templates.show_logo":       "Show logo",
	"templates.show_qr":         "Show QR code",
	"templates.show_notes":      "Show notes",
	"templates.updated":         "Template updated",
	"templates.qr_heading":      "QR CODE SETTINGS",
	"templates.select_account":  "Select bank account",
	"templates.qr_spayd_desc":   "For Czech bank accounts",
	"templates.qr_pbs_desc":     "Slovak format",
	"templates.qr_epc_desc":     "EU standard",
	"templates.qr_none":         "No QR code",
	"templates.select_qr_type":  "Select QR code type",
	"templates.qr_updated":      "QR code type updated",

	// ── Custom templates ──────────────────────────────────────────────
	"templates.duplicate":            "Duplicate template",
	"templates.export_yaml":          "Export YAML",
	"templates.import_yaml":          "Import YAML",
	"templates.delete_custom":        "Delete custom template",
	"templates.show_ai_prompt":       "Show AI prompt",
	"templates.select_to_duplicate":  "Select template to duplicate:",
	"templates.select_to_export":     "Select template to export:",
	"templates.new_name":             "New template name",
	"templates.duplicate_created":    "Template '%s' created (ID: %s)",
	"templates.no_yaml_source":       "Template has no YAML source",
	"templates.save_to_file":         "Save to file (empty = no)",
	"templates.file_saved":           "Saved",
	"templates.yaml_file_path":       "Path to YAML file (or 'stdin' for stdin input)",
	"templates.paste_yaml":           "Paste YAML (end with Ctrl+D):",
	"templates.invalid_yaml":         "Invalid YAML",
	"templates.no_default_found":     "Cannot find default template",
	"templates.import_created":       "Template '%s' imported (ID: %s)",
	"templates.no_custom_templates":  "No custom templates to delete.",
	"templates.custom_templates":     "Custom templates:",
	"templates.confirm_delete":       "Really delete template '%s'?",
	"templates.template_deleted":     "Template '%s' deleted",

	// ── Logo & Directories ─────────────────────────────────────────────
	"action.set_logo":            "L) Set logo",
	"prompt.logo_path":           "Path to logo file (PNG/JPG)",
	"success.logo_uploaded":      "Logo uploaded",
	"success.logo_deleted":       "Logo deleted",
	"error.logo_upload":          "Failed to upload logo: %v",
	"error.logo_invalid_type":    "Unsupported file type (PNG and JPG only)",
	"error.logo_too_large":       "File is too large (max 2 MB)",
	"error.logo_not_found":       "Logo not found",
	"label.logo":                 "Logo:    %s",
	"label.logo_set":             "set",
	"label.logo_not_set":         "not set",
	"confirm.delete_logo":        "Delete current logo?",

	"action.change_directories":  "D) Change directories",
	"heading.directories":        "DIRECTORY SETTINGS",
	"label.dir_logos":            "Logo directory:     %s",
	"label.dir_pdfs":             "PDF directory:      %s",
	"label.dir_previews":         "Preview directory:  %s",
	"prompt.dir_logos":           "Logo directory",
	"prompt.dir_pdfs":            "PDF directory",
	"prompt.dir_previews":        "Preview directory",
	"success.directories_updated": "Directories updated",
	"error.directory_invalid":    "Cannot create directory: %v",

	// ── Units ──────────────────────────────────────────────────────────
	"settings.add_unit_or_type": "  or type a custom unit",
	"settings.manage_units":    "Manage units",
	"settings.units_title":     "MANAGE UNITS",
	"settings.add_unit":        "Add unit",
	"settings.remove_unit":     "Remove unit",
	"settings.set_default_unit": "Set default unit",
	"success.units_updated":    "Units updated",

	// ── Payment Types ──────────────────────────────────────────────────
	"prompt.taxable_date_confirm":      "Taxable supply date (DD.MM.YYYY)",
	"payment_type.bank_transfer":       "Bank transfer",
	"payment_type.cash":                "Cash",
	"pdf.cash":                         "Cash",
	"settings.manage_payment_types":    "Manage payment types",
	"settings.payment_types_title":     "PAYMENT TYPES",
	"settings.add_payment_type":        "Add payment type",
	"settings.remove_payment_type":     "Remove payment type",
	"settings.set_default_payment_type": "Set default type",
	"success.payment_types_updated":    "Payment types updated",
	"settings.add_payment_type_or_type": "  or type a custom payment type",

	// ── Errors (new) ────────────────────────────────────────────────────
	"error.invalid_option":  "Invalid option",
	"error.no_bank_accounts":      "No bank accounts",
	"bank_account.add_new":        "Create new bank account",
	"error.builtin_cannot_delete": "Built-in type, cannot be deleted",

	// ── PDF Template Names ──────────────────────────────────────────────
	"template.name.table": "Tables",
	"template.name.classic": "Classic",
	"template.name.modern":  "Modern",
	"template.name.minimal": "Minimal",
	"template.desc.table": "Template with gray header and full table borders",
	"template.desc.classic": "Traditional black and white invoice with page numbers and formal layout",
	"template.desc.modern":  "Contemporary design with steel blue accent and color hierarchy",
	"template.desc.minimal": "Minimalist style with maximum white space and no decorations",

	// ── Preview ─────────────────────────────────────────────────────────
	"preview.notes":         "This is a sample document. Thank you for your trust and we look forward to further cooperation.",
	"preview.supplier_name": "Sample Company Ltd.",
	"preview.customer_name": "Test Customer Inc.",
	"preview.item1":         "Web application development",
	"preview.item2":         "Consulting and requirements analysis",
	"preview.item3":         "Server administration (monthly)",

	// ── Payment / Bank ──────────────────────────────────────────────────
	"prompt.requires_bank_info": "Requires bank info? (Y/n)",
	"prompt.variable_symbol":    "Reference number",
	"label.requires_bank_info":  "Requires bank info",
	"label.no_bank_info":        "no bank",

	// ── Sync / Import / Export ──────────────────────────────────────────
	"sync.export_all":              "Export all (backup)",
	"sync.export_filtered":         "Export with filter",
	"sync.import_merge":            "Import (merge)",
	"sync.import_replace":          "Import (replace all)",
	"sync.import_force":            "Import (force overwrite)",
	"sync.import_preview":          "Import preview",
	"sync.prompt_output_file":      "Output file",
	"sync.prompt_input_file":       "Input file (.tidybill)",
	"sync.exporting":               "Exporting data...",
	"sync.importing":               "Importing data...",
	"sync.export_done":             "Export saved: %s",
	"sync.import_done":             "Import complete.",
	"sync.preview_done":            "Preview complete (no changes were made).",
	"sync.invalid_file":            "Invalid backup file: %v",
	"sync.filter_by_supplier":      "Filter by supplier?",
	"sync.no_suppliers":            "No suppliers found.",
	"sync.pick_suppliers":          "Select suppliers (comma-separated numbers)",
	"sync.filter_skip_old_paid":    "Skip paid invoices older than N years?",
	"sync.skip_years":              "Number of years",
	"sync.filter_exclude_settings": "Exclude settings?",
	"sync.count_suppliers":         "Suppliers:",
	"sync.count_customers":         "Customers:",
	"sync.count_invoices":          "Invoices:",
	"sync.count_items":             "Items:",
	"sync.count_templates":         "Templates:",
	"sync.count_settings":          "Settings:",
	"sync.file_size":               "File size:",
	"sync.meta_exported_at":        "Exported at:",
	"sync.meta_device_id":          "Device ID:",
	"sync.meta_format_ver":         "Format version:",
	"sync.meta_app_ver":            "App version:",
	"sync.meta_mode":               "Export mode:",
	"sync.warn_replace":            "WARNING: This will DELETE all existing data and replace with the backup!",
	"sync.warn_force":              "WARNING: This will overwrite existing records regardless of conflicts!",
	"sync.confirm_replace":         "Are you sure you want to replace all data?",
	"sync.confirm_force":           "Are you sure you want to force overwrite?",
	"sync.conflict":                "Conflict",
	"sync.warning":                 "Warning",
	"sync.table_suppliers":         "Suppliers",
	"sync.table_bank_accounts":     "Bank Accounts",
	"sync.table_customers":         "Customers",
	"sync.table_items":             "Items",
	"sync.table_invoices":          "Invoices",
	"sync.table_invoice_items":     "Invoice Items",
	"sync.table_settings":          "Settings",
	"sync.table_vat_rates":         "VAT Rates",
	"sync.table_templates":         "PDF Templates",
	"sync.table_smtp":              "SMTP Configs",
	"sync.table_cust_items":        "Customer Items",
	"sync.col_table":               "Table",
	"sync.col_inserted":            "Inserted",
	"sync.col_updated":             "Updated",
	"sync.col_skipped":             "Skipped",

	// ── About ───────────────────────────────────────────────────────────
	"menu.about":          "About",
	"heading.about":       "ABOUT TIDYBILL",
	"about.version":       "Version: %s",
	"about.description":   "Simple, private invoicing for freelancers and small businesses.",
	"about.opensource":    "TidyBill is free and open-source software.",
	"about.issues_title":  "Found a bug or want a feature?",
	"about.issues_url":    "Report it at: https://github.com/adamSHA256/tidybill/issues",
	"about.support_title": "Support the author",
	"about.support_desc":  "If you find TidyBill useful, you can support its development:",
	"about.monero":        "Monero (XMR): <42GYPXCvn42NbsujN8wRVrQj4xLnWubXm4BJmiyZjkb8PuGCMb75iC96BHkia6LJM57BfVqyGJm2VH3Mr97c269hRxSidqG>",
	"about.bitcoin":       "Bitcoin (BTC): <bc1q5dqj6jfpuq47qvmu2w3awt7jz4zlyyfcyreayx>",
}
