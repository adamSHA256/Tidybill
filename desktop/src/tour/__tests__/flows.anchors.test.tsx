...
function renderAtRoute(route: string) {
  ...
  // createMemoryRouter is required (not MemoryRouter) because InvoiceCreate uses useBlocker
  // which only works in data routers.
...
describe('flows anchors', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })

  for (const flow of Object.values(ALL_FLOWS)) {
    for (const step of flow.steps) {
      it(`flow "${flow.id}" anchor "${step.anchor}" exists at route "${step.route ?? '/'}"`, async () => {
        renderAtRoute(step.route ?? '/')

        // Wait for the anchor element to appear — some pages render a <Loader> while queries
        // are pending (Dashboard, CustomerList, SupplierList, ItemCatalog, Templates, Settings).
        await waitFor(
          () => {
